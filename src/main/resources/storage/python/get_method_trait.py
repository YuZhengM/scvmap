...
def get_trait_info(trs_file, trait_id, cell_rate, metadata):
    cell_rate = float(cell_rate)
    data = ad.read(trs_file)
    trait_data = data[:, trait_id]

    column_name = "f_" + metadata

    # get indexs
    data_cell_type_size = data.obs.groupby(column_name).size()
    data_index_list = []

    for i in data_cell_type_size.index.values:
        cell_count = int(np.ceil(data_cell_type_size[i] * cell_rate))
        cell_index = list(data.obs[(data.obs[column_name] == i) & (data.obs[column_name + "_index"] < cell_count)].index)
    ...
    print(trait_data.obs[column_name].tolist())
...
if __name__ == '__main__':
    get_trait_info(sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4])
