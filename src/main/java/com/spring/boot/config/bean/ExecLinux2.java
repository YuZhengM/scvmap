 ...
 */
@Data
@ConfigurationProperties(prefix = "com.linux2")
public class ExecLinux2 {

    private static final Log log = LogFactory.getLog(ExecLinux2.class);
    ...
    public ExecLinux2() {
        ...
        if (StringUtil.isEmpty(host)) {
            host = readConfig.getConfiguration("com.linux2.ip");
        }
        if (NumberUtil.isEqualZero(port)) {
            port = Integer.parseInt(readConfig.getConfiguration("com.linux2.port"));
        }
        if (StringUtil.isEmpty(username)) {
            username = readConfig.getConfiguration("com.linux2.username");
        }
        if (StringUtil.isEmpty(password)) {
            password = readConfig.getConfiguration("com.linux2.password");
        }
        if (StringUtil.isEmpty(keyFile)) {
            keyFile = readConfig.getConfiguration("com.linux2.keyFile");
        }
    }
